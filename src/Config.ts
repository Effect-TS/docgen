...
const readJsonFile = (path: string): Effect.Effect<never, Error, unknown> =>
  pipe(
    ...
    Effect.flatMap(
      Either.liftThrowable(JSON.parse, (e) =>
...
const parsePackageJson = pipe(
  Process.cwd,
  Effect.flatMap((cwd) =>
    parseJsonFile(NodePath.join(cwd, "package.json"), PackageJSONSchema)
  )
);

const getConfigPath = Effect.map(Process.cwd, (cwd) =>
  NodePath.join(cwd, "docs-ts.json")
);
  ...
  Effect.ifEffect(
    ...
    pipe(
      ...
      Effect.flatMap((configPath) =>
        parseJsonFile(configPath, PartialConfigSchema)
      ),
    ...
    pipe(
      Logger.info(
        chalk.bold(
          "No configuration file detected, using default configuration"
        )
      ),
